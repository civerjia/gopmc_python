...
__kernel void initParticles(__global PS * particle, float T, float2 width, float3 sourceCenter, float3 sourceDir, float m, float c, int randSeed){
...
//	}
	...
	iseed[1] = gid;
	jswRand(iseed);
	particle[gid].pos.x = sourceCenter.x+(MTrng(iseed) - 0.5f) * width.s0;
	particle[gid].pos.y = sourceCenter.y+(MTrng(iseed) - 0.5f) * width.s1;
	
	//particle[gid].dir = normalize((float3)(0.0f, 0.0f, 0.0f) - sourceCenter);
	particle[gid].dir = normalize(sourceDir);
...
inline float totalLinearSigma(float4 vox, read_only image2d_t MCS, float e) {
	...
	#endif
	return mcs.s0*vox.s3 + (mcs.s1 + mcs.s2 + mcs.s3)*vox.s2; // missing element weight? Lin
}
...
void hardEvent(PS * thisOne, float stepLength, float4 vox, read_only image2d_t MCS, read_only image2d_t MSPR, float material, global float8 * doseCounter,
	...
	float sigPPE = mcs.s1*vox.s2; // These three lines
	float sigPOE = mcs.s2*vox.s2; // may have error.
	float sigPOI = mcs.s3*vox.s2; // Shouldn't we consider element weight?
